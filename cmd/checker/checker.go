...
func checkResult(cdrs []utils.CDR) {
	...
	cdr1 := utils.CDR{
		...
		DateToWrite: "2019-12-02 18:00:29 +0000 UTC",
	}
	cdr2 := utils.CDR{
		RecordType: 18000,
		...
		MscNumber:             "77070200005",
		DateToWrite: "2019-12-02 18:00:36 +0000 UTC",
	}
	cdr3 := utils.CDR{
		RecordType: 18000,
		...
		DateToWrite: "2019-12-02 18:00:41 +0000 UTC",
	}
	cdr4 := utils.CDR{
		RecordType: 18000,
		...
		DateToWrite: "2019-12-02 18:01:23 +0000 UTC",
	}
	cdr5 := utils.CDR{
		RecordType: 18000,
	...
	for i, _ := range tc {
		if cdrs[i].CallingPartyNumber != tc[i].CallingPartyNumber {
			log.Fatalf("Err calling_party mismatch got - %s, expected %s at TEST %d" , cdrs[i].CallingPartyNumber, tc[i].CallingPartyNumber, i)
		}
		if cdrs[i].CalledPartyNumber != tc[i].CalledPartyNumber {
			log.Fatalf("Err called_party mismatch got - %s, expected %s at TEST %d", cdrs[i].CalledPartyNumber, tc[i].CalledPartyNumber, i)
		}
		if cdrs[i].ImsiCalled != tc[i].ImsiCalled {
			log.Fatalf("Err imsi_called mismatch, got - %s, expected %s at TEST %d", cdrs[i].ImsiCalled, tc[i].ImsiCalled, i)
		}
		if cdrs[i].MscNumber != tc[i].MscNumber {
			log.Fatalf("Err msc_num mismatch, got - %s, expected %s at TEST %d", cdrs[i].MscNumber, tc[i].MscNumber, i)
		}
		// actual date written to db from CSv, to all fields as well
		if cdrs[i].StartTimestamp.String() != tc[i].DateToWrite {
			log.Fatalf("Err date_and_time mismatch, got - %s, expected %s at TEST %d", cdrs[i].StartTimestamp.String(), tc[i].DateToWrite, i)
		}
	}
}
